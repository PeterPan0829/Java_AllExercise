...
/*
    ...
    Linear_Search() 循序搜尋功能是搜尋到想要處理資料的位置。
    Binary_Search() 二分搜尋法功能是要刪除元素，用二分搜尋法比較適合因為要先找到你想要的學號刪除元素，Bubble_sort() 功能是依照平均分數排列。
...
public class Array_Sequence_GradeSystem {

    static int flag, low, high, mid;
    ...
    static void print_course() {
        ...
        for (int i=0; i<=point; i++) {                                   //顯示科目後再顯示分數
            for (int j=0; j<course[i].length; j=j+1) {
    ...
    static int Binary_Search(int value) {
        ...
        while ((low + 1) < high) {
            mid = (low + high) / 2;
            if (value == course[mid][0]) {                              //比對陣列中間元素
                flag = 1;                                               //成立的話，表示找到該資料，則設定旗標並中斷離開。
                break;
            } else if (value > course[mid][0]) {                         //在陣列的後半段。如未找到，但比中間值 a[mid] 大的話，則設定搜尋後半段（low = mid，但 high 未改變）。
                low = mid;
            } else                                                       //在陣列的前半段
                high = mid;                                              //都不是的話，則設定搜尋前半段（low 未改變）。
        }
//        if (flag == 1) {
//            System.out.printf("學號 %d 成績是 %d\n", course[mid][0], course[mid][1]);
//        } else {
